...
def show_report_settings(api):
    ...
    if 'report_expander_key' not in st.session_state:
        st.session_state.report_expander_key = 0

    # השתמש ב-key דינמי כדי לאלץ reset של ה-expander
    with st.expander("⚙️ הגדרות דוח (לחץ להסתרה/הרחבה)",
                     expanded=st.session_state.report_settings_expanded,
                     key=f"report_settings_exp_{st.session_state.report_expander_key}"):
...
def show():
    ...
    if search_clicked or 'history_report_data' in st.session_state:
        if search_clicked:
            # סגירת expander של הגדרות דוח + עדכון key לאפס את המצב
            st.session_state.report_settings_expanded = False
            st.session_state.report_expander_key += 1
            # קריאת נתונים מ-API
